...
시스템의 상태(State)와 전이(Transition)를 표현한다. **8.1 전체 시스템 상태 머신**으로 사용자 여정 전체를 조망하고, 이후 도메인별 상세 상태 머신으로 내려간다.
...
```
- **피드백**: `(user_id, image_id)` UNIQUE 1행 — 같은 이미지에 LIKE/DISLIKE는 upsert로 토글, 해제는 삭제. 효과는 비대칭 — **싫어요**는 다음 검색에서 해당 이미지를 제외하고 3회 누적 시 생성 유도 모달을 띄우며, **좋아요**는 반응만 저장(정렬·유지용)해 검색 랭킹에는 반영되지 않는다.
