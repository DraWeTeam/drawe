...
DraWe의 주요 기능인 텍스트 기반 가이드에서 **한국어 자유 요청 → 정확한 레퍼런스 추천·미술 조언**을 만드는 AI 파이프라인이다. 이 한 끗 가이드는 **관찰 → 진단 → 결정 → 검색 → 코칭 → 피드백**으로 이어지는 코칭 에이전트 파이프라인이다.
...
### 5.2.1 전체 의도 라우팅 (IntentRouting.ROUTING)
...
- **Live 게이트**: live는 **COMPOSE 종착 의도만 허용**(§5.9) — GENERATE처럼 COMPOSE로 끝나지 않는 의도는 부팅 검증에서 막힌다.
