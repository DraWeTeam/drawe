...
| **External** | **Pinecone**(채팅 추천 벡터) · **Qdrant Cloud**(가이드 벡터) · LLM(Grok 코칭·Claude) · **VLM 관찰**(env `VLM_BACKEND`: dev/local Gemini `aistudio` / prod Bedrock Claude Haiku 4.5) · **이미지 생성**(env `AI_GEN_PROVIDER`: dev Gemini / prod Bedrock Stability) · Google OAuth | 외부 HTTPS, Resilience4j로 격리. 관찰·생성 env는 별개 |
