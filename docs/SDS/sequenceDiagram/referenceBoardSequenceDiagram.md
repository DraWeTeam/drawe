...
sequenceDiagram
    ...
    Client->>Ctl: GET /reference-board/search?q&source&topK
...
sequenceDiagram
    ...
    Client->>Ctl: POST /reference-board/generate {prompt}
    ...
    Note over Client,Ctl: 보드 재진입 시 GET /generations → generationHistory(시간순, url 재서명)로 생성 채팅 복원
```
...
sequenceDiagram
    ...
    alt 좋아요
        Client->>Ctl: POST /images/{imageId}/like
        ...
        Client->>Ctl: POST /images/{imageId}/dislike
        ...
        opt count >= 3 (생성 유도 모달)
            Client->>Ctl: POST /generation-suggestion/ack (모달 노출 후)
...
| AI 생성 (⭐) | `generateReference` → `imageGenerationService.generate`(→ `GuideClient` → **Bedrock Stability**) → `ReferenceGeneration` 이력 저장 → `{imageId, signed(url)}` | `@Transactional`, 소유 검증(NOT_FOUND/FORBIDDEN). 생성 대화(프롬프트 500자→이미지)를 저장해 보드 재진입 시 `GET /generations`로 채팅 복원(SCRUM-118) |
