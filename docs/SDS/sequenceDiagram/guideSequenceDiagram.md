...
sequenceDiagram
    ...
    participant FastAPI as fastapi-guide (외부)<br/>OpenCLIP·mediapipe·Qdrant·LLM
    ...
    alt 멱등 히트 — existing.isPresent() (재시도 dedup)
        ...
        Note over FastAPI: OpenCLIP → observation + mediapipe(손 VLM) → Qdrant 검색 → LLM 코칭 합성<br/>request_id 로 부작용 at-most-once 디둡
...
sequenceDiagram
    ...
    Note over Client,GuideClient: ② 레퍼런스 채택 — POST /{guideId}/references/feedback
        ...
        loop refId in targets
            ...
            Note over GuideClient: POST /adopt<br/>(guide_id, reference_id, event) → adoption_log 적재<br/>레퍼런스 취향을 guide 서비스에 반영. timeout 5s, best-effort(실패 삼킴)
...
| 외부 반영(adopt) | 채택 대상 ref마다 `GuideClient.adopt(guideId, refId, event)` → fastapi-guide `POST /adopt`. | body `{guide_id, reference_id, event}` → `adoption_log` 적재로 취향 반영. timeout 5s, best-effort(실패해도 예외 삼키고 `log.warn`만, 사용자 흐름 유지). 임의 ref 적재는 페이로드 풀 화이트리스트로 차단. |
