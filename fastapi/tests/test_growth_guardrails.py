...
sys.path.insert(0, str(pathlib.Path(__file__).resolve().parents[1]))  # fastapi/ → guide 임포트
...
def _kp(vis=0.9):
    kp = [(0.5, 0.5, vis)] * 33
    pts = {0: (0.5, 0.18), 11: (0.42, 0.30), 12: (0.58, 0.30), 23: (0.44, 0.55),
           24: (0.56, 0.55), 13: (0.36, 0.42), 14: (0.64, 0.42), 15: (0.32, 0.55),
           16: (0.68, 0.55), 25: (0.45, 0.74), 26: (0.55, 0.74), 27: (0.46, 0.95),
           28: (0.54, 0.95)}
...
def _spatial(tier):
    ...
    if tier == POSE_OK:
        return {"keypoints": _kp(), "bbox": (0.30, 0.12, 0.70, 0.98),
                "centroid": (0.52, 0.55), "horizon_y": 0.66}
    if tier == POSE_LOW:
        return {"keypoints": None, "bbox": (0.30, 0.12, 0.70, 0.98),
                "centroid": (0.52, 0.55), "horizon_y": 0.66}
    return {"keypoints": None, "bbox": None, "centroid": (0.52, 0.55), "horizon_y": 0.66}


# ── 1. 내부 stage / 평가어 누출 금지 ──────────────────────────────────────────────────────
def test_stage_constants_are_internal_strings():
    # 불변식의 대상이 실제로 이 세 문자열임을 고정(누구도 슬쩍 노출 라벨로 바꾸지 못하게)
    assert (growth_stage.FOUNDATION, growth_stage.DEVELOPING, growth_stage.REFINING) == (
...
def test_clean_payload_has_no_false_positive():
    payload = {"one_thing": "원경 명암을 한 단계 눌러보세요", "blocks": [{"observation": "구도"}]}
...
def test_overlay_has_no_marks_for_unmeasured():
    obs = [{"sub_problem": "composition_balance", "measured": True},
           {"sub_problem": "light_direction", "measured": False}]
    svg = build_overlay(800, 1000, resolve_anchors(obs, _spatial(POSE_OK), POSE_OK), POSE_OK)
...
def test_unmeasured_observation_has_empty_signal():
    ...
    prop = next((o for o in dx["observations"] if o["sub_problem"] == "proportion"), None)
...
def test_pose_reliability_tiers():
    ...
    assert pose_reliability({"status": "low_confidence", "keypoints": _kp()}) == POSE_LOW
...
def test_visual_mode_recurring_goes_overlay():
    ...
    m = select_visual_mode(obs, {"cold": False, "steady": [], "recurring": ["composition_balance"]})
