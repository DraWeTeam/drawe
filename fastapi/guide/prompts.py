...
- 각 블록의 네 부분은 서로 달라야 한다(같은 문장·표현 반복 금지):
  ...
  · direction(방향): 지금 바로 해볼 수 있는 실험 한 가지(실험형). 단 '추천 연습'(focus_practice)과
    층위가 달라야 한다 — 연습이 '무엇을 그릴지(행동)'면 direction은 이 그림에서 '무엇을 확인할지
    (관찰 가능한 체크) 한 가지'다. 같은 동작을 말만 바꿔 반복하지 마라(예: 연습이 '3단계로 나눠
    그려보기'면 direction은 '나눈 세 단계가 실제로 서로 구분되는지' 확인).
...
def build_coach_prompt(
        ...
        "observations": [
            {
                ...
                "signal": o.get("signal", ""),
                "what_to_observe": o["what_to_observe"],
                "practice_prompt": o["practice_prompt"],
            }
    ...
    if growth:
        ...
        target = next(
            (
                c
                for c in (growth.get("current_focus"), growth.get("next_goal"))
                if c and c in LABELS and c not in improved
            ),
            None,
        )
        data = {}
        ...
        if improved:
            ...
            data["overall"] = (
                "최근 그림들 전반에서 약점으로 잡히던 부분의 수가 줄어드는 경향"
            )
        if data:
            grow = (
    ...
    if next_steps and getattr(next_steps, "focus", None):
        from guide.pipeline.roadmap import LABELS  # 지연 import

        # reason_code → 코치가 말할 법한 '이유' 힌트(룰/에이전트가 정한 사실 → LLM 은 어투만 입힘).
        _RH = {
            "recent_recurring": "최근 진단에 이 부분이 자주 보여 이어서 다지는 흐름",
        ...
        nf = {
            "focus": LABELS.get(next_steps.focus, next_steps.focus),
            "focus_practice": next_steps.focus_practice,
            "next_goal": (
                LABELS.get(next_steps.next_goal, next_steps.next_goal)
                if next_steps.next_goal
                else None
            ),
        ...
        nxt = (
            ...
            "마라). (1-1) note 는 focus_practice 의 동작 문구를 그대로 되풀이하지 마라 — '왜·맥락·전이'만 "
            "담아라(연습은 별도 카드로 따로 보인다). (2) 해석·평가 금지 — '좋아졌어요/성장/실력/졸업/잘함' 류 절대 금지(관찰·배열만). "
