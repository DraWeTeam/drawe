...
for f in sorted(imgs):
    ...
    rows.append(
        (f, lab["clarity"], it, auto, score(auto, lab), tracked, score(tracked, lab))
    )
...
for f, clr, it, auto, sa, tr, st in rows:
    print(
        f"{f:38} {clr:6} {str(it):16} {str(auto):22} {'O' if sa else 'X'} {str(tr):22} {'O' if st else 'X'}"
    )
...
print(
    f"애매셋({len(amb)}) auto: {sum(r[4] for r in amb)}/{len(amb)}  tracked: {sum(r[6] for r in amb)}/{len(amb)}"
)
