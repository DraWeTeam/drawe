...
for sp in AXES:
    ...
    print(
        f"  AXIS_PREF       : {assets.AXIS_PREF.get(sp, '(없음 → _DEFAULT_PREF=' + str(assets._DEFAULT_PREF) + ')')}"
    )
    print(f"  in AI_AVOID?    : {sp in assets.AI_AVOID}")
    print(f"  in POSE_DEPENDENT?: {sp in assets.POSE_DEPENDENT}")
    print(
        f"  _FLOOR_CAPTION  : {assets._FLOOR_CAPTION.get(sp, '(없음 → _GENERIC_CAPTION)')}"
    )
    ...
    print(
        f"  floor_svg len={len(svg)} generic={'_GENERIC_SVG' if sp not in assets._FLOOR_SVG else 'dedicated'}"
    )
