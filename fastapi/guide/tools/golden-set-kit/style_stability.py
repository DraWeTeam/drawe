...
def clip_style(pil):
    ...
    return keys[i], float(probs[i]), {k: round(sc[v], 3) for k, v in profiles._STYLE_LABELS.items()}


def main():
    print(f"STYLE_VLM={os.environ.get('STYLE_VLM')}  _STYLE_HIGH={profiles._STYLE_HIGH}  N={N}")
    print("=" * 100)
    ...
    for fn, (truth_style, axis) in TRUTH.items():
        ...
        band = "CLIP확신(>=0.70)" if cconf >= profiles._STYLE_HIGH else "에스컬레이션(<0.70)"
        ...
        print(f"  CLIP : top={ctop:10s} conf={cconf:.3f}  [{band}] {ok_mark} probs={cprobs}")
        print(f"  VLM   draws (N={N}): {dict(vc)}")
        print(f"  RESOLVE draws (발화/abstain): {dict(rc)}  fired={fired}/{N}  -> {stable}")
    print("\n" + "=" * 100 + "\ndone")
