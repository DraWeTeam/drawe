...
## 메타데이터 (4단계, 블라인드 라벨과 분리 저장)
생성 의도가 블라인드 라벨에 새지 않게 **별도 파일** `expansion_meta.json`(키=파일명):
`{ source:"ai_gen", model, prompt_cell, gen_intent, style }`. ground truth 라벨은 기존 `labels.json`.
→ 나중에 "AI셋 정확도 vs 실사용 정확도"로 도메인 갭 추적.
