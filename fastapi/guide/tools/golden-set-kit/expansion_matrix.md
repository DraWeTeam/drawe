...
## 영역3 관절(joint_articulation) = CLOSED (음성, 카드 이전 단계에서 종결)
**카드 짓기 전 발화 실측 = 음성 확정.** 코드+labels 그라운딩, 삼중 차단:
1. **데이터:** `labels.json`에 joint_articulation을 expected_primary/acceptable로 가진 이미지 **0장** — 발화를 잴 양성 케이스 자체가 없음.
2. **검출:** `s_joint_articulation`은 BlazePose 팔꿈치/무릎 각(과신전 >177°) 필요 → pose D 게이트(figure 4/6 미검출), 검출된 2장도 과신전 아님 → 발화 0.
3. **원칙 차단:** weight_balance/action_line을 살린 VLM 관찰자 우회로가 **positive-only(0003)에 막힘** — 관절은 *결함축*(음성)이라 `_vlm_pose_signal`이 신호를 안 내고(diagnose 329-333), 게이트 예외(diagnose 771)도 action_line·weight_balance만. 영역2(observe_body_shape=중립 서술)처럼 "관찰자 하나 더"가 **안 됨**.
→ 영역3은 검출+데이터에 게이트 = §3-0②(섀도우 계측)·골든 확장(관절 테스트 이미지)이 푸는 문제. 카드 작업 아님.

## 영역4 무게중심(weight_balance) = 카드 GO (b 균형교정, known-limitation 인정)
weight_balance 축은 **균형 붕괴 감지**(쏠림 → 지지발 위로 안정화, taxonomy:5,8 / com_offset>0.9 키포인트 + VLM balance=불안정). 카드 **(b) plumb-line 확정·배포(`b9551f7`)** — 골반 무게중심에서 내린 수직선이 지지면(발) 안에 떨어지는지.
- **알려진 한계:** VLM 경로가 *좋은 콘트라포스토*와 *진짜 불균형*을 미구분(둘 다 "한 발 지지·쏠림"으로 보임) + 골든셋에 콘트라포스토 대조 이미지 0장 → over-fire **미측정**. 카드 경계 줄("균형 붕괴 교정용·콘트라포스토는 별개 개념")로 유해성 중화(정적 개념교습이라 작가가 스스로 적용범위 거름). 빈도는 §3-0②(섀도우)/③(골든 확장)에서 측정.
- **(a) 콘트라포스토 "비대칭 추가"는 미포함** — 축 방향과 거꾸로(축은 쏠림을 *줄이는* 방향), action_line(s_action_line: torso_lean<0.03=직립 감지) 별건.

### 영역4 콘트라포스토 통제 대조군 = CLOSED (여섯 경로 음성/한계, 2026-06-30)
weight_balance over-fire 통제용 "잘 그린 안정 콘트라포스토 vs 진짜 불균형" 대조군 조달 시도 종료. 여섯 경로 전부 음성/한계:
1. **합성(AI 생성)** — ground truth 부재(imagen-4.0: "한 발 체중"→다리들기 직역, 안정 콘트라포스토 1/5, 불안정과 시각 미구분).
2. **SVG 규칙 생성** — 측정 대상을 규칙으로 그리면 기준선이 순환(측정 대상→기준선 순환).
3. **3D 백본 재활용(a')** — 콘트라포스토 콘텐츠 부재(백본은 정적=대칭·비대칭=동적 이분, stable-asymmetry 없음).
4. **action 클립 B** — 정적 스탠스도 비대칭 아님(동적만 비대칭).
5. **직접 포징** — 노동 한계 + 원본 씬 미보존 → COM 자동 라벨 불가.
6. **이미지→3D 생성(Meshy류)** — 리그 미생성→COM 불가, 단일뷰 추측이 비대칭 변형, PNG→3D→2D 순환.

**근본 원인 = stable-asymmetry 양성군의 통제 조달이 구조적으로 어려움**(finger-proportion CLOSED 동류). weight_balance 축은 **정상 작동**(균형 붕괴 감지), 카드 (b) plumb-line 배포 + 경계 줄("균형 붕괴 교정용·콘트라포스토 별개")로 over-fire 유해성 **중화 완료**. **over-fire 빈도 측정 = ②섀도우(실트래픽 작가 그림, 도메인 정합·무노동) 단독 이관.** 빈도 높으면 observe_pose에 'COM 지지발 위면 침묵' 게이트 추가 경로 존재(ADR 0004 패턴). 수기 콘트라포스토 PNG는 카드 예시·②스팟체크용 보존(측정 비사용).
