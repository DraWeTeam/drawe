...
from guide.stores.s3 import put_image, put_svg
...
def ingest(
    ...
    *,
    ...
    vstore.upsert(
        ref_id, vec.tolist(), payload
    )  # 백엔드 중립(어댑터) — qdrant/pinecone 무관

    with engine.begin() as cx:
        cx.execute(
            text("""INSERT INTO reference_images
          ...
          VALUES (:r,:k,:tk,:sk,:st,:lic,:at,:ok,:p,:t,:em,:w,:h,:rp,:rg,:cat,:bt,:gd)"""),
            dict(
                ...
                at=attribution,
                ok=commercial_ok,
                p=json.dumps(personas),
                t=json.dumps(tags),
                em=embedder.model_id,
                w=pil.width,
                h=pil.height,
