...
def _reference_dir():
    ...
    cands = [
        os.environ.get("REFERENCE_DIR"),
        "/repo/assets/reference",  # docker: ./:/repo:ro 마운트
        os.path.join(
            here, "..", "..", "assets", "reference"
        ),  # 소스: artref/assets/reference
        os.path.join(here, "..", "..", "..", "woz", "public", "reference"),
    ]  # repo/woz/public/reference
...
def _load_reference_index():
    ...
    if d:
        try:
            ai = json.load(
                open(os.path.join(d, "manifest.json"), encoding="utf-8")
            ).get("asset_index", {})
...
def _load_index():
    ...
    try:
        ...
        with engine.begin() as cx:
            (cnt,) = cx.execute(
                text(
                    "SELECT COUNT(*) FROM reference_images WHERE source_type='self_render'"
                )
            ).fetchone()
            if cnt == _CACHE["count"]:
                return _CACHE["index"]
...
def _ai_candidates_from_rows(rows):
    ...
    """
    from guide.pipeline.assets import AI_AVOID
...
def _load_ai_index():
    ...
    try:
        ...
        with engine.begin() as cx:
            rows = cx.execute(
                text(
                    "SELECT ref_id, tags FROM reference_images WHERE source_type='ai_example'"
                )
            ).fetchall()
