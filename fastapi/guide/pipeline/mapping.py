...
log = logging.getLogger("drawe-fastapi.guide.pipeline.mapping")

BASE_TERMS = {
    "pose": ["dynamic pose"],
    "anatomy": ["anatomy reference"],
    "hand": ["hand reference"],
    "light": ["lighting study"],
    "color": ["color palette"],
...
def log_miss(term, context=None):
    ...
    try:
        with engine.begin() as cx:
            upd = cx.execute(
                text(
                    "UPDATE miss_log SET count = count + 1, "
        ...
        log.warning(f"[miss] 로깅 실패(무시): {type(e).__name__}: {e}")
