...
def pose_signals(kp):
    ...
    legs = (
        np.linalg.norm(_xy(kp, L_HIP) - _xy(kp, L_AN))
        + np.linalg.norm(_xy(kp, R_HIP) - _xy(kp, R_AN))
    ) / 2
    ...
    return {
        ...
        "arm_proj_ratio": float(min(armL, armR) / max(armL, armR))
        if max(armL, armR)
        else 1.0,
        "elbow_angle_min": min(
            _angle(kp, L_SH, L_EL, L_WR), _angle(kp, R_SH, R_EL, R_WR)
        ),
        "knee_angle_min": min(
            _angle(kp, L_HIP, L_KN, L_AN), _angle(kp, R_HIP, R_KN, R_AN)
        ),
    }
...
def _subject_value_range(g):
    ...
    *밝은 배경(종이) 위에서만* 신뢰한다 — 어두운 배경이면 피사체 그림자가 배경과 같은 밝기라
    ...
    if (
        h[mode] / g.size < 0.30
    ):  # 지배적 배경 없음(피사체가 꽉 참) → 마스크 없이 신뢰 불가
        return None
...
def light_signals(pil, pose):
    ...
    small = (
        np.asarray(_Img.fromarray((g * 255).astype("uint8")).resize((16, 16)), float)
        / 255
    )
...
def s_weight_balance(s):
    o = s.get("com_offset")
    t = _T("weight_balance.com_offset")
    if o and o > t:
        return (min(0.65, 0.3 + 0.35 * (o - t)), f"무게중심이 지지면 밖 ≈ {o:.1f}배")


def s_foreshortening(s):
    r = s.get("arm_proj_ratio")
    t = _T("foreshortening.arm_proj_ratio")
    if r is not None and r < t:
        return (min(0.6, 0.3 + 0.5 * (t - r)), f"좌우 팔 투영 길이비 ≈ {r:.2f}")


def s_proportion(s):
    r = s.get("leg_torso_ratio")
    band = (s.get("_norms") or {}).get(
        "leg_torso", (0.75, 1.7)
...
def s_action_line(s):
    lean = s.get("torso_lean")
    t = _T("action_line.torso_lean")
    if lean is not None and lean < t:
        return (0.3, f"몸통 기울기 ≈ {lean:.02f} (거의 직립)")


def s_joint_articulation(s):
    vs = [
        v for v in (s.get("elbow_angle_min"), s.get("knee_angle_min")) if v is not None
    ]
...
def s_value_structure(s):
    ...
    if fr is not None:  # 인물 내부 명도 폭(배경 대비에 속지 않음)
        ...
        if fr < t:
            ...
            parts.append(
                f"인물 영역의 밝은 곳과 어두운 곳 차이가 좁음(명도 폭 ≈ {fr:.2f})"
            )
    ...
    if (
        bg is not None and bg < tb
    ):  # 인물-배경 명도차 작음 → 실루엣이 배경과 섞임(보수적 임계)
        conf = max(conf, min(0.5, 0.3 + 0.5 * (tb - bg)))
        parts.append(
            f"인물과 배경의 명도 차가 작아(차 ≈ {bg:.2f}) 실루엣이 배경과 섞여 보임"
        )
    if parts:
...
def s_color_harmony(s):
    ...
    if (
        sm is not None
        and hs is not None
        and sm > _T("color_harmony.sat_mean")
        and hs > _T("color_harmony.hue_spread")
    ):
...
def s_light_direction(s):
    ...
    if r is not None and r < _T("light_direction.light_ramp"):
        return (
            0.3,
            f"화면 전반의 밝기 변화가 작아(≈{r:.3f}) 광원 방향이 뚜렷하지 않음",
        )
...
def s_atmospheric_perspective(s):
    ...
    if f is not None and f < _T("atmospheric_perspective.falloff_min"):
        return (
            0.3,
            f"원경과 근경의 대비 차가 작아(차 ≈ {f:.2f}) 거리감이 평면적으로 보일 수 있음",
        )
...
SCORERS = {
    ...
    "action_line": s_action_line,
    "joint_articulation": s_joint_articulation,
    "value_structure": s_value_structure,
    "composition_balance": s_composition_balance,
    "color_harmony": s_color_harmony,
    "light_direction": s_light_direction,
