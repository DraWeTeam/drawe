...
class DummyLLM:
    def complete_json(self, prompt: str, step: str = "coach") -> str:
        # step 은 RealLLM 과 시그니처 정합용(오프라인 템플릿은 계측 대상 아님 — 실 LLM 지연 없음).
        del step
...
class RealLLM:
    ...
    def complete_json(self, prompt: str, step: str = "coach") -> str:
        ...
        # step(plan|select|coach) 라벨로 호출별 지연을 계측(drawe_llm_latency) — 어느 호출이 지배적인지.
        trace("llm.in", prompt_chars=len(prompt), provider="grok", step=step)
        t0 = time.monotonic()
        if not self.key:
            observe_llm(time.monotonic() - t0, step=step, outcome="fallback")
        ...
        try:
            ...
            observe_llm(time.monotonic() - t0, step=step, outcome="success")
            trace(
                "llm.out",
                fallback=False,
                out_chars=len(out),
