...
from guide.stores.s3 import ensure_bucket, presigned_url
...
from guide.pipeline.roadmap import get_roadmap, record_practice, growth_context, _why, growth_view
...
from guide._security import (valid_ref_id, clean_event, clamp_confidence, cors_origins,
                       PRACTICE_ACTIONS)
from guide._auth import is_authorized
from guide._ratelimit import Limiter
...
def _claim_request(request_id, guide_id):
    ...
    try:
        with engine.begin() as cx:
            r = cx.execute(
                text("INSERT IGNORE INTO guide_request (request_id, guide_id) VALUES (:rid, :gid)"),
...
async def guide_ep(file: UploadFile, message: str = Form(""),
    ...
    if resp.mode == "coach":
        ...
        growth_obj = growth_from_raw(growth_view(user_id, track=track, degraded=resp.degraded), note=_note)
