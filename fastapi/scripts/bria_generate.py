...
_BRIA_BASE = os.environ.get(
    "BRIA_API_BASE", "https://engine.prod.bria-api.com/v1/text-to-image/base"
)
...
def _bria_image(token, model, prompt):
    ...
    if not img_url:
        print(
            "    이미지 URL 을 못 찾음 — _extract_url() 을 현재 Bria 응답 구조에 맞게 수정하세요."
        )
        return None
    try:
...
def main():
    ...
    ap.add_argument(
        "--model",
        default=os.environ.get("BRIA_MODEL", "2.3"),
        help="Bria base 모델 버전(또는 BRIA_MODEL 환경변수). 기본 2.3",
    )
    ap.add_argument(
        "--sleep", type=float, default=0.0, help="요청 간 대기(초) — 레이트리밋 회피용"
    )
    args = ap.parse_args()
    ...
    print(
        f"다음: python scripts/ingest_ai_examples.py {args.out}   ← QC 게이트 통과분만 적재"
    )
