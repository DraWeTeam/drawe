...
실행(컨테이너 — pose 가 mediapipe, 신호가 PIL/numpy):
  docker compose exec -w /app guide python scripts/extract_signals.py /repo/labeled_imgs > out.jsonl
...
from guide.pipeline import diagnose as D
...
def main():
    ...
    for fn in files:
        try:
            ...
            print(
                json.dumps(
                    {"name": fn, "error": f"{type(e).__name__}: {e}"},
                    ensure_ascii=False,
                ),
                file=sys.stderr,
            )
