...
def main():
    ...
    check(
        "/adopt 정상 event 는 400 아님", r.status_code != 400, f"status={r.status_code}"
    )

    # B-4: 잘못된 action
    r = requests.post(
        f"{BASE}/practice",
        json={"user_id": "u", "sub_problem": "proportion", "action": "NOPE"},
        timeout=5,
    )
    check(
        "/practice 잘못된 action → 400", r.status_code == 400, f"status={r.status_code}"
    )
