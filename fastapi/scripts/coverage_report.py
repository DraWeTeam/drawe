...
실행:
    docker compose exec -w /app guide python scripts/coverage_report.py
"""
...
def main():
    with engine.begin() as cx:
        ...
        cnt = Counter()
        for personas_json, st in cx.execute(
            text("SELECT personas, source_type FROM reference_images")
        ):
            try:
    ...
    print(
        f"{'sub_problem':20}{'persona':10}{'노출':>6}{'채택':>6}{'CTR':>7}"
    ...
    print(
        "  (노출 수요는 큰데 self_render가 적거나 CTR이 낮은 영역이 먼저 올라옵니다.)"
    )
