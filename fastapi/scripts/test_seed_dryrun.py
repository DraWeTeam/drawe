...
OBJECTS = {
    101: {
        "objectID": 101,
        "isPublicDomain": True,
        "primaryImage": "https://img/101.jpg",
    ...
    102: {
        ...
        "primaryImage": "https://img/102.jpg",
        "title": "T102",
    },
    103: {"objectID": 103, "isPublicDomain": True, "primaryImage": "", "title": "T103"},
    104: {
        "objectID": 104,
        "isPublicDomain": True,
        "primaryImage": "https://img/104.jpg",
    ...
    105: {
        "objectID": 105,
        "isPublicDomain": True,
        "primaryImage": "https://img/105.jpg",
    ...
    106: {
        "objectID": 106,
        "isPublicDomain": False,
        "primaryImage": "https://img/106.jpg",
        "title": "T106",
    },
    107: "RAISE",
}
...
class _FakeRequests:
    def get(self, url, params=None, timeout=None):
        if "/search" in url:
            ...
            return _Resp(
                payload={"total": len(SEARCH_IDS), "objectIDs": list(SEARCH_IDS)}
            )
        if "/objects/" in url:
...
def main():
    ...
    ok &= check(
        "T1 persona 태그 == ['light','mood']",
        all(p == ["light", "mood"] for p in personas_all),
    )
    pils = {pil for pil, _ in INGEST_CALLS}
    ok &= check(
        "T1 적재된 것은 CC0+이미지(101/104/105)뿐, 102/103/106 제외",
        pils == {b"IMG101", b"IMG104", b"IMG105"},
    )
    meta_ok = all(
        kw["source_type"] == "museum"
        and kw["license"] == "CC0"
        and kw["commercial_ok"] is True
        for _, kw in INGEST_CALLS
    )
    ok &= check("T1 메타(source_type=museum, license=CC0, commercial_ok=True)", meta_ok)

    # ── T2: hasImages 파라미터가 소문자 'true' ──
    ok &= check(
        "T2 search params hasImages == 'true' (소문자)",
        all(p.get("hasImages") == "true" for p in SEARCH_PARAMS)
        and len(SEARCH_PARAMS) == 2,
    )
    ...
    ok &= check("T5 6개 카테고리 모두 등장", len(cats) == 6)
