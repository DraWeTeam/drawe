...
실행(컨테이너, /repo 마운트):
  docker compose exec -w /app guide python scripts/clear_self_render.py          # dry-run(건수만)
  docker compose exec -w /app guide python scripts/clear_self_render.py --yes     # 실제 삭제
"""
...
def main():
    ...
    with engine.begin() as cx:
        n = cx.execute(
            text("SELECT COUNT(*) FROM reference_images WHERE source_type=:s"),
            {"s": SOURCE},
        ).scalar()
    ...
    with engine.begin() as cx:
        cx.execute(
            text("DELETE FROM reference_images WHERE source_type=:s"), {"s": SOURCE}
        )
