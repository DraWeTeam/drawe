...
실행(컨테이너 안, /repo 기준):
    docker compose exec -w /app guide python scripts/render_audit.py
...
from guide.pipeline._searchlogic import CONSTRUCTION_PERSONAS, SOURCE_PREF
...
def section_inventory():
    ...
    try:
        ...
        print(
            f"  (region/category 컬럼 조회 실패 — 마이그레이션 002 미적용 가능: "
            f"{type(e).__name__}) → render_params에서 가능한 것만 집계"
        )
        with engine.begin() as cx:
            rows = [
                (None, None, None, None, p, rp)
                for p, rp in cx.execute(
                    text(
    ...
    print(
        f"  체형 종류: {n_body}개"
        + ("  ← 2개 이하면 비율/해부 다양성 얇음" if n_body <= 2 else "")
    )
    styles = [
        k
        for k in body
        if k and any(s in str(k).lower() for s in ("anime", "chibi", "toon"))
    ]
    print(
        f"  스타일형 체형(anime/chibi 등): {len(styles)}개"
        + (
            "  ← 0개면 애니/치비 사용자에 맞는 레퍼런스 없음(알려진 갭)"
            if not styles
            else ""
        )
    )
    print(
        f"  hand region 보유: {region.get('hand', 0)}건 (수는 보지만 손가락 품질은 눈으로 확인 필요)"
    )
    print(
        "  ※ azimuth는 Qdrant payload엔 없고 MySQL에만 있음 → 현재 시점 단위 라우팅 불가(E단계 참고)"
    )
...
def probe(query, persona):
    ...
    must_not = (
        {"source_type": "ai_example"} if persona in CONSTRUCTION_PERSONAS else None
    )
    try:
    ...
    facets = {
        (h.meta.get("body_type"), h.meta.get("region"), h.meta.get("category"))
        for h in hits
        if h.meta
    }
...
def section_battery():
    ...
    for sp, e in tax.items():
        ...
        for fam, rows in per_family.items():
            ...
            print(
                f"{sp:20}{tflag:7}{fam:7}{len(rows):>3}{miss * 100:>6.0f}%"
...
def section_telemetry():
    ...
    try:
        ...
        print(
            f"  (텔레메트리 조회 실패/미구성: {type(e).__name__}) — 트래픽 전이면 정상, 건너뜀"
        )
        return
    if not shown:
