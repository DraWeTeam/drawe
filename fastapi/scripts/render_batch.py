...
읽을 수 있는 곳에 둔다(예: artref/renders → /repo/renders, /repo는 읽기전용 마운트):
    docker compose exec -w /app guide python scripts/render_batch.py /repo/renders
...
CATEGORY_PERSONAS = {
    "action": ["pose", "anatomy"],
    "standing": ["pose", "anatomy"],
    "sitting": ["pose", "anatomy"],
    "other": ["pose", "anatomy"],
}
...
def _reset_self_render(state_path):
    ...
    try:
        with engine.begin() as cx:
            cx.execute(
                text("DELETE FROM reference_images WHERE source_type='self_render'")
            )
...
def main():
    ...
    ap.add_argument(
        "--state",
        default=os.path.join(tempfile.gettempdir(), "render_batch_state.jsonl"),
    )
    ap.add_argument(
        "--reset",
        action="store_true",
    ...
    if not os.path.exists(manifest):
        ...
        sys.exit(1)
    rows = [
        json.loads(line) for line in open(manifest, encoding="utf-8") if line.strip()
    ]
    ...
    state_f.close()
    print(
        f"완료: 적재 {n_ok}, 스킵(기존) {n_skip}, 이미지없음 {n_miss}. "
        f"상태파일: {args.state}"
    )
