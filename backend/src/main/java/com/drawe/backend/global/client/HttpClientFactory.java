 ...
 *
 * <p>기본 {@code RestClient.create()} 는 타임아웃이 없어 외부 API 가 hang 하면 톰캣 스레드가 무한 대기 → 풀 고갈로 서비스 전체가 마비된다.
 * 모든 동기 RestClient(Bria, LLM 3종)는 이 팩토리로 생성해 타임아웃을 강제한다. 설계: {@code
...
public final class HttpClientFactory {
  ...
  /**
   * 지정한 타임아웃을 적용한 {@link RestClient} 를 생성한다.
   *
