...
public class CostPricingProperties {
  ...
  public BigDecimal estimateUsd(String providerName, Long promptTokens, Long completionTokens) {
    ...
    BigDecimal inCost = p.getInputPer1m().multiply(BigDecimal.valueOf(in)).divide(million);
    BigDecimal outCost = p.getOutputPer1m().multiply(BigDecimal.valueOf(out)).divide(million);
