 ...
 *
 * <p>정적 전략 맵 패턴(ADR §3): 동적 PlanSpec 인터프리터 대신 {@code IntentCode → List<StepType>} 정적 맵을 lookup 해
 * 실행. 각 {@link StepExecutor} 는 스프링이 타입별로 자동 수집해 {@code Map<StepType, StepExecutor>} 로 주입한다 — 새 step
 * 추가 = enum + Executor 빈 등록만.
 *
 * <p><b>현재(트랙 A ③) 상태</b>: WorkflowService 와 Executor 골격은 준비됐으나 {@code ChatLlmService.chat()} 에 아직
 * 연결하지 않는다. 실연결은 ② 경량 LLM 분류기(IntentResult 생성)와 모든 step 의 Executor 가 갖춰진 뒤. 그전까지는 단위 테스트로만 검증한다.
 *
 * <p>step 실패 정책: 한 step 이 던진 예외가 전체 파이프라인을 깨지 않도록 잡아서 로그하고 **현재까지 누적된 컨텍스트를 그대로 반환**한다(부분 성공). 이미
 * 채워진 슬롯(예: keywords 까지 됐고 search 실패)은 보존된다.
 */
@Slf4j
@Service
public class WorkflowService {
  ...
  /**
   * StepExecutor 빈들을 {@code type()} 키로 묶어 워크플로 실행기를 구성한다.
   *
