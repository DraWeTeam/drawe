 ...
 * 칩 분석 집계 (읽기 전용, 네이티브).
 *
 * <p>노출 = {@code analytics_events} 의 {@code chip_shown} payload.chips 를 JSON_TABLE 로 unnest. 반영 =
 * {@code projects.keywords}(JSON 배열)를 JSON_TABLE 로 unnest. 라벨 정규화·조인·전환율은 서비스(순수 함수)에서 한다.
 *
