 ...
 *
 * <p>노출 = {@code chip_shown} 이벤트(WP6-a), 반영 = {@code projects.keywords}(JSON). 라벨
 * 정규화(lowercase·trim)로 조인한다. <b>정렬은 노출 보정 전환율</b>(반영/노출) — 많이 추천된 칩이 무조건 위로 가지 않게(어절 랭킹과 같은 함정 방지).
 *
...
public record ChipModel() {
  ...
  public record View(
    ...
    public String koReflectText() {
      return koReflectRate == null ? "—" : String.format(Locale.US, "%.0f%%", koReflectRate * 100);
    }
  }
