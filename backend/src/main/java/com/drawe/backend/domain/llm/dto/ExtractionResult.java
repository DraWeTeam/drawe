...
public record ExtractionResult(
  ...
  public enum Action {
    ...
    GENERATE_NOW,
    /**
     * 직전 ASSISTANT 답변에 대한 부연·후속 질문 (012 FOLLOWUP). KEEP(레퍼런스 유지)과 달리 "방금 한 말을 더 설명/재설명/평가해달라"는 요청이라
     * 검색을 안 하고 직전 답변을 이어서 풀어준다. 베타에서 "더 설명"/"말로 설명"/ "어때?" 류가 KEEP/SKIP 으로 뭉개지며 'AI 생성 권유' 오답이 반복된
     * 게 만족도 저하의 직접 원인이었다.
     */
    FOLLOWUP,
    /**
     * 이미 대화에 나온 대상(레퍼런스 [1] vs [2], 직전 답변에 언급한 것들)을 말로 비교해달라는 요청 (013 COMPARE). FOLLOWUP 의 사촌 —
