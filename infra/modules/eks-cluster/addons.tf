############################################################
# Core 애드온: vpc-cni(★Prefix Delegation) / kube-proxy / coredns / ebs-csi
############################################################

# ── vpc-cni — Prefix Delegation + WARM ──
# managed addon 의 configuration_values 로 선언적 설정(가장 idiomatic).
...
# coredns 는 Deployment → 스케줄될 노드가 필요 → system NG 이후 생성
