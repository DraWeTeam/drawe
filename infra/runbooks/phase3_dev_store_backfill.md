...
순서: **베이스(`ddl.sql`) → 증분(`002` … `012`)**. (001 은 `ddl.sql` 이 대신함.)

```bash
SCHEMA=fastapi/guide/schema
DEV_DB="mysql -h <rds-host> -u drawe_guide -p drawe_guide"

# 1) 베이스 스키마 (reference_images / adoption_log / miss_log …)
$DEV_DB < $SCHEMA/ddl.sql

# 2) 증분 마이그레이션 (번호 순)
for f in $(ls $SCHEMA/migrations/0*.sql | sort); do
  echo ">> $f"
  $DEV_DB < "$f"
done
```
...
(a)~(d) 가 모두 통과하면 **P3 종료**. 가이딩 응답의 `4. 추천 레퍼런스` 썸네일이 dev 에서 실제로 뜬다.

---

## 6. 코드 측 적응(이번 통합에서 반영된 것 — 참고)

P5 배포(IAM task role)로 굴리기 위해 런타임 코드를 다음과 같이 맞춰 두었다:
...
- prod Qdrant 컬렉션도 **768(ViT-L/14)** 로 시드돼 있어야 한다(§5 게이트 동일 적용).
