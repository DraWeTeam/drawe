############################################################
# fastapi-guide — 이미지 가이딩 전용 ECS 서비스 (P10 prod 승격)
#
# terraform-dev/ecs-guide.tf 와 동일 구조. prod 계정/컬렉션/버킷/RDS 로만 바뀐다.
...
#   s3-artref.tf / ssm-qdrant.tf / ssm-artref-fastapi.tf / ssm.tf 의 prod 리소스를 재사용한다.
# IAM 은 공용 task role(aws_iam_role.ecs_task: artref S3 RW) + exec role(SSM read)을 그대로 쓴다.
# SG 는 ecs_fastapi(백엔드→8000 ingress) 재사용.
#
# prod capacity: 단일 ec2 capacity provider 를 쓰되, ASG(mixed-instances)가
#   on-demand 베이스라인(warm standby) + spot 을 섞는다 → 여기 리소스는 dev 와 동일.
############################################################
...
variable "fastapi_guide_desired_count" {
  description = "guide 서비스 desired count. prod 는 warm standby 위해 >=1 유지."
  type        = number
  default     = 1
}

variable "guide_hand_vlm" {
  description = "손 VLM 게이트. dev 검증(P9) 통과 후 prod 도 동일 절차로 0→1 해제."
  type        = string
  default     = "0"
}
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = concat([
        ...
        { name = "HAND_VLM", value = var.guide_hand_vlm }, # prod 도 동일 절차 0(다크)→1
...
resource "aws_ecs_service" "fastapi_guide" {
  ...
  # 단일 ec2 capacity provider. prod ASG(mixed-instances)가 on-demand base + spot 을 섞는다.
